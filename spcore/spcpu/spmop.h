...
namespace sp{
	...
	SP_CPUCALL void setMem(Mem<TYPE> &dst, const Mem<TYPE> &mem0){
	...
	SP_CPUCALL void cnvMem(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const double scale = 1.0, const double base = 0.0){
		dst.resize(mem0.dim, mem0.dsize);
	...
	SP_CPUCALL void addMem(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const Mem<TYPE1> &mem1){
	...
	template<typename TYPE, typename TYPE0, typename TYPE1>
	SP_CPUCALL void subMem(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const Mem<TYPE1> &mem1){
		if (cmpSize(mem0, mem1) == false) return;
	...
	template<typename TYPE, typename TYPE0, typename TYPE1>
	SP_CPUCALL void mulMem(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const Mem<TYPE1> &mem1){
		if (cmpSize(mem0, mem1) == false) return;
	...
	template<typename TYPE, typename TYPE0, typename TYPE1>
	SP_CPUCALL void divMem(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const Mem<TYPE1> &mem1){
		if (cmpSize(mem0, mem1) == false) return;
	...
	SP_CPUCALL void setElm(Mem<TYPE> &dst, const ELEM &elm){
	...
	SP_CPUCALL void addElm(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const ELEM &elm){
	...
	template<typename TYPE, typename TYPE0, typename ELEM>
	SP_CPUCALL void subElm(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const ELEM &elm){
	...
	template<typename TYPE, typename TYPE0, typename ELEM>
	SP_CPUCALL void mulElm(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const ELEM &elm){
	...
	template<typename TYPE, typename TYPE0, typename ELEM>
	SP_CPUCALL void divElm(Mem<TYPE> &dst, const Mem<TYPE0> &mem0, const ELEM &elm){
	...
	template<typename TYPE, typename ELEM>
	SP_CPUCALL void addElm(Mem<TYPE> &dst, const ELEM &elm){
		addElm(dst, dst, elm);
	}

	template<typename TYPE, typename ELEM>
	SP_CPUCALL void subElm(Mem<TYPE> &dst, const ELEM &elm){
		subElm(dst, dst, elm);
	}

	template<typename TYPE, typename ELEM>
	SP_CPUCALL void mulElm(Mem<TYPE> &dst, const ELEM &elm){
		mulElm(dst, dst, elm);
	}

	template<typename TYPE, typename ELEM>
	SP_CPUCALL void divElm(Mem<TYPE> &dst, const ELEM &elm){
		divElm(dst, dst, elm);
	}
	...
	SP_CPUCALL Mem1<TYPE> range(const int start, const int stop, const int step = 1) {
	...
	template<typename TYPE>
	SP_CPUCALL Mem<TYPE> shuffle(const Mem<TYPE> &src, const int seed = 0) {
	...
	template<typename TYPE, typename ELEM>
	SP_CPUCALL Mem1<TYPE> operator + (const Mem1<TYPE> &mem0, const ELEM &elm){
		Mem1<TYPE> dst;
		addElm(dst, mem0, elm);
		return dst;
	}
	...
	template<typename TYPE, typename ELEM>
	SP_CPUCALL void operator += (Mem1<TYPE> &dst, const ELEM &elm){
		addElm(dst, elm);
	}
	...
	template<typename TYPE, typename ELEM>
	SP_CPUCALL Mem2<TYPE> operator + (const Mem2<TYPE> &mem0, const ELEM &elm) {
		Mem2<TYPE> dst;
		addElm(dst, mem0, elm);
		return dst;
	}
	...
	template<typename TYPE, typename ELEM>
	SP_CPUCALL void operator += (Mem2<TYPE> &dst, const ELEM &elm) {
		addElm(dst, elm);
	}
	...
	SP_CPUCALL Mat getMat(const Vec2 &vec) {
		Mat dst(2, 1);

		return dst;
	}

	SP_CPUCALL Mat getMat(const Vec3 &vec) {
		Mat dst(3, 1);
		return dst;
	}

	SP_CPUCALL Mat getMat(const Rot &rot) {
		Mat dst(3, 3);
	...
	SP_CPUCALL Mat getMat(const Pose &pose) {
		Mat dst(3, 4);
	...
	SP_CPUCALL Mat getMat(const CamParam &cam) {
		Mat dst(3, 3);
	...
	SP_CPUCALL Mat eyeMat(const int rows, const int cols){
		Mat dst(rows, cols);
	...
	SP_CPUCALL Mat zeroMat(const int rows, const int cols){
		Mat dst(rows, cols);
	...
	SP_CPUCALL Mat extMat(const int rows, const int cols, const Mat &mat){
		Mat dst(rows, cols);
	...
	SP_CPUCALL Mat trnMat(const Mat &mat){
	...
	SP_CPUCALL Mat covMat(const Mat &mat){
	...
	SP_CPUCALL Mat skewMat(const Vec3 &vec){
		Mat dst(3, 3);
		skewMat(dst.ptr, 3, 3, vec);
		return dst;
	}

	SP_CPUCALL double normMat(const Mat &mat){
		return normMat(mat.ptr, mat.rows(), mat.cols());
	}

	SP_CPUCALL double detMat(const Mat &mat){
		Mat buf(mat.dsize);
	...
	SP_CPUCALL bool eigMat(Mat &eigVec, Mat &eigVal, const Mat &mat, const bool minOrder = true){
	...
	SP_CPUCALL bool svdMat(Mat &U, Mat &S, Mat &V, const Mat &mat, const bool minOrder = true){
		Mat m;
	...
	SP_CPUCALL void mulMat(Mat &dst, const Mat &mat0, const Mat &mat1){
	...
	SP_CPUCALL void addMat(Mat &dst, const Mat &mat0, const Mat &mat1){
	...
	SP_CPUCALL void subMat(Mat &dst, const Mat &mat0, const Mat &mat1){
		if (mat0.rows() == mat1.rows() && mat0.cols() == mat1.cols()){
			dst.resize(mat0.dsize);
	...
	SP_CPUCALL Mat operator * (const Mat &mat0, const Mat &mat1){
		Mat dst;
	...
	SP_CPUCALL void operator += (Mat &dst, const Mat &mat0){
		addMat(dst, dst, mat0);
	}
	...
	SP_CPUCALL Mat operator + (const Mat &mat0, const double val){
		Mat dst;
		addElm(dst, mat0, val);
		return dst;
	}
	...
	SP_CPUCALL void operator += (Mat &dst, const double val){
		addElm(dst, dst, val);
	}
	...
	SP_CPUCALL Vec2 operator * (const Mat &mat, const Vec2 vec){
	...
	SP_CPUCALL VecVN3 operator * (const Mat &mat, const VecVN3 vec){
		return mulMat(mat.ptr, mat.rows(), mat.cols(), vec);
	}

	SP_CPUCALL Mesh operator * (const Mat &mat, const Mesh mesh){
	...
	SP_CPUCALL Mem<Vec3> getVec(const Mem<Vec2> &vec, const double z) {
	...
	SP_CPUCALL Mem2<Vec2> grid(const int dsize0, const int dsize1) {
