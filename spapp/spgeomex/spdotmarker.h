...
namespace sp{
    ...
    private:
        ...
        SP_REAL getSearchRange(){
            return max(m_cam.dsize[0], m_cam.dsize[1]) / 2.0;
        }
        ...
        void estimate(const Mem2<Byte> &img, const Mem1<Vec2> &pixs){
...

            {
                ...
                if (max(rcgEval, trkEval) < 0.6 * m_mrk.map.dsize[0] * m_mrk.map.dsize[1]){
                    throw "eval";
                }
        ...
        void getBlob(Mem1<Vec2> &pixs, const Mem2<Byte> &orgImg, const Mem2<Byte> &minImg, const Mem2<int> &labelMap, const SP_REAL minScale){
            ...
            for (int i = 0; i < rects.size(); i++) {
                ...
                // check size
                const int maxSize = round(min(minImg.dsize[0], minImg.dsize[1]) * DOT_MAXSIZE);
                if (max(minRect.dsize[0], minRect.dsize[1]) > maxSize) continue;
...

        {
            ...
            for (int v = 0; v < img.dsize[1]; v++) {
                for (int u = 0; u < img.dsize[0]; u++) {
                    ...
                    const SP_REAL rw = max(0.0, fabs(vec.x) - dw);
                    const SP_REAL rh = max(0.0, fabs(vec.y) - dh);
                    const SP_REAL rate = 2 * max(rw, rh);
