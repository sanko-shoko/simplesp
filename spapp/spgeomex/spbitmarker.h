			...
			// calc pose
			{
				...
				for (int i = 0; i < corners.size(); i++) {
					...
					for (int c = 0; c < unit.size(); c++){
						const Vec2 pix = m_cam * npxDist(m_cam, prjVec(pose * unit[c]));
			...
			// calc z rotation
			{
				...
				for (int i = 0; i < poses.size(); i++) {
					...
					for (int j = 0; j < 4; j++){
						...
						const Vec2 pixA = m_cam * npxDist(m_cam, prjVec(pos + drc * 0.1));
						const Vec2 pixB = m_cam * npxDist(m_cam, prjVec(pos - drc * 0.1));
						const Vec2 pixC = m_cam * npxDist(m_cam, prjVec(pos));
			...
			for (int p = 0; p < pimgs.size(); p++){
				...
				for (int v = 0; v < dsize[1]; v++){
					for (int u = 0; u < dsize[0]; u++){
						...
						const Vec2 pix = cam * npxDist(cam, prjVec(pose * obj));
		...
		bool calcBase(Pose &base, const Mem1<Pose> &poses) {
			...
			for (int i = 0; i < poses.size(); i++){
				...
				for (int c = 0; c < unit.size(); c++){
					...
					const Vec2 pix = m_cam * npxDist(m_cam, prjVec(pose * obj));
					objs.push(obj);
					pixs.push(pix);
				}
				vposes.push(pose);
