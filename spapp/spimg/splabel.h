...
namespace sp{
    ...
    SP_CPUFUNC int labeling(Mem2<int> &map, const Mem2<TYPE> &img, const TYPE thresh, const bool near8 = false) {

        const int step = img.dsize[0];

        map.resize(img.dsize);

        Mem1<int> table;
        table.reserve(img.size());
